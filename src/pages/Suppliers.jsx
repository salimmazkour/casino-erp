...
export default function Suppliers() {
      ...
      const suppliersWithBalances = suppliersData.map(supplier => {
        ...
        const overdueAmount = supplierOrders.reduce((sum, order) => {
          ...
          if (orderReceptions.length > 0) {
            ...
            const dueDate = calculateDueDate(latestReception.reception_date, supplier.payment_terms);
            if (dueDate) {
  ...
  function editSupplier(supplier) {
    ...
    setFormData({
      ...
      payment_terms: supplier.payment_terms || '30 jours fin de mois',
              ...
              <tbody>
                {filteredSuppliers.map(supplier => (
                  <tr key={supplier.id} className={!supplier.is_active ? 'inactive-row' : ''}>
                    ...
                    <td className="payment-terms">{supplier.payment_terms}</td>
